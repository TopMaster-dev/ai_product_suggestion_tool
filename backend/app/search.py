...
- 条件リストにはシノニム（テフロン → フッ素加工 等）を OR 展開して当てる
- 厳密検索で 0 件のときは「条件 → カテゴリ」の順に段階的に緩めて再検索する
- ORDER BY は 重点商品区分・発注ランク を優先（補助シグナルがあればそれを上に積む）
"""
...
def _LIKE_シノニム条件(対象カラム一覧: List[str], 値: str) -> Tuple[str, List[str]]:
    """
    与えた値とその同義語(シノニム辞書)を OR でまとめて LIKE する。
    例：「テフロン加工」→「テフロン加工 / フッ素加工 / ふっ素加工 ...」を OR で検索。
    """
    候補語: List[str] = list(dict.fromkeys(条件シノニム辞書.get(値, [値])))
    if 値 not in 候補語:
        候補語.insert(0, 値)
...
def _補助シグナル句構築(フィルター: dict) -> Tuple[str | None, List[Any]]:
    ...
    for キーワード in フィルター.get("キーワードリスト") or []:
        節, 値 = _LIKE_シノニム条件(columns.キーワード検索カラム一覧, str(キーワード))
        句一覧.append(節)
        パラメータ.extend(値)
...
def _検索SQL組み立て(
    ...
    *,
    ...
    """WHERE句と並び順を組み立てて (SQL, params) を返す"""
    ...
    # ── カテゴリ（AND：もっとも強い絞り込み）──────
    カテゴリ指定あり = bool(フィルター.get("カテゴリ")) and カテゴリを含める
    if カテゴリ指定あり:
        節, 値 = _LIKE_条件(
    ...
    # ── 条件リスト（複数 AND、各条件はシノニムOR）─────
    if 条件を含める:
        for 条件キーワード in フィルター.get("条件リスト", []) or []:
            節, 値 = _LIKE_シノニム条件(columns.条件検索カラム一覧, str(条件キーワード))
            WHERE条件.append(節)
            WHEREパラメータ.extend(値)
    ...
    if 補助句 and not WHERE用補助あり:
        ORDER一覧.append(f"({補助句}) DESC")
        ORDERパラメータ.extend(補助パラメータ)

    ORDER一覧.append(columns.ランクORDER_SQL)
    ORDER一覧.append("`有効在庫数` DESC, `アスパック登録日` DESC")
    ORDER_BY = ", ".join(ORDER一覧)
    ...
    SQL = f"""
        ...
        LIMIT {取得件数}
    """
    return SQL, WHEREパラメータ + ORDERパラメータ


def 商品検索(フィルター: dict) -> List[dict]:
    """フィルターから動的にWHEREを組み立てて商品を取得する（フォールバック付き）"""
    ...
    # ── 段階的な検索戦略 ──
    # 1) すべての条件 (カテゴリ+条件) で検索
    # 2) 0件なら 条件 を外して再試行（=カテゴリのみ+補助シグナル）
    # 3) それでも 0件なら カテゴリ も外して再試行（=補助シグナルのみ）
    検索戦略 = [
        (True,  True),   # 条件あり、カテゴリあり
        (False, True),   # 条件なし、カテゴリあり
        (False, False),  # 条件なし、カテゴリなし（補助シグナルのみ）
    ]

    for 条件を含める, カテゴリを含める in 検索戦略:
        SQL, パラメータ = _検索SQL組み立て(
            フィルター,
            条件を含める=条件を含める,
            カテゴリを含める=カテゴリを含める,
        )
